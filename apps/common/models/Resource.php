...
class Resource extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('uid, storage', 'required'),
			array('date_created, date_modified, uid', 'numerical', 'integerOnly'=>true),
			array('name, path', 'length', 'max'=>255),
			array('type, storage', 'length', 'max'=>50),
			array('body', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, body, path, type, date_created, date_modified, uid, storage', 'safe', 'on'=>'search'),
		);
	}
