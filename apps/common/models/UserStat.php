...
class UserStat extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, actor_id, action, ip, user_agent, date_created', 'required'),
			array('user_id, actor_id, date_created', 'numerical', 'integerOnly'=>true),
			array('action', 'length', 'max'=>20),
			array('ip', 'length', 'max'=>50),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, actor_id, action, ip, user_agent, date_created', 'safe', 'on'=>'search'),
		);
	}
